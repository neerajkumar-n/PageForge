  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        ...
        {maxLength && (
          <span className={['text-xs font-mono', over ? 'text-red-500' : 'text-zinc-500'].join(' ')}>
      ...
      {multiline ? (
        <textarea
          ...
          rows={3}
          className={['w-full text-sm border rounded-xl px-3 py-2 resize-none outline-none transition-colors', over ? 'border-red-300 focus:border-red-400' : 'border-zinc-800 focus:border-amber-400'].join(' ')}
        />
      ) : (
        <input
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className={['w-full text-sm border rounded-xl px-3 py-2 outline-none transition-colors', over ? 'border-red-300 focus:border-red-400' : 'border-zinc-800 focus:border-amber-400'].join(' ')}
        />
      )}
    </div>
  )
...
function SERPPreview({ title, description, url }: { title: string; description: string; url: string }) {
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
      <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-3">SERP Preview</p>
      ...
      <p className="text-sm text-zinc-500 line-clamp-2 leading-snug mt-0.5">
  ...
  if (status === 'idle' || status === 'error') {
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-[300px] gap-4 text-center">
        <div className="w-16 h-16 rounded-2xl bg-amber-950/20 flex items-center justify-center text-2xl">🔍</div>
        <div>
          <p className="font-semibold text-zinc-100">SEO Agent</p>
        ...
        <Button onClick={handleRun} disabled={isRunning || !canRun} className="bg-amber-950/200 hover:bg-amber-600">
  ...
  if (status === 'running' || isRunning) {
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-[300px] gap-4 text-center">
        ...
        <p className="font-semibold text-zinc-100">SEO Agent running...</p>
        <p className="text-sm text-zinc-500">Generating meta tags, schema, and keyword strategy</p>
      </div>
    )
  }
    ...
    <div className="p-5 space-y-6 pb-24">
      ...
      <div className="space-y-4 p-4 bg-zinc-900 border border-zinc-800 rounded-xl">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-semibold text-zinc-100">Search Tags</p>
        </div>
      ...
      <div className="space-y-4 p-4 bg-zinc-900 border border-zinc-800 rounded-xl">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-semibold text-zinc-100">Social Sharing</p>
        </div>
      ...
      <div className="space-y-3 p-4 bg-zinc-900 border border-zinc-800 rounded-xl">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-semibold text-zinc-100">Keywords</p>
        </div>
        <div>
          <p className="text-xs text-zinc-500 mb-1">Focus Keyword</p>
          <span className="inline-block bg-amber-100 text-amber-400 text-sm font-semibold px-3 py-1 rounded-full">
            {seo.focusKeyword}
          </span>
        </div>
        <div>
          <p className="text-xs text-zinc-500 mb-2">All Keywords</p>
          <div className="flex flex-wrap gap-1.5">
            {seo.keywords.map((kw, i) => (
              <span key={i} className="text-xs bg-zinc-800 text-zinc-400 px-2 py-1 rounded-full">{kw}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="p-4 bg-zinc-900 border border-zinc-800 rounded-xl">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-zinc-100">Schema Markup</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs bg-green-100 text-green-400 px-2 py-0.5 rounded-full flex items-center gap-1">
              <CheckCircle size={10} /> {seo.schemaType}
            </span>
            <span className="text-xs text-zinc-500">{schemaOpen ? '▲' : '▼'}</span>
          </div>
        </button>
        {schemaOpen && (
          <pre className="mt-3 text-xs bg-zinc-950 border border-zinc-800 rounded-lg p-3 overflow-x-auto text-zinc-300 leading-relaxed">
